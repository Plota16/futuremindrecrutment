...
class ReferenceRepository:
    ...
    def _upsert(self, model, name_attr: str, id_attr: str,
                names: Iterable[str]) -> dict[str, int]:
        existing = {getattr(r, name_attr): getattr(r, id_attr) for r in
                    self.session.exec(select(model)).all()}
        new = [model(**{name_attr: n}) for n in sorted({x for x in names if x})
    ...
    def upsert_distributors(self, names: Iterable[str]) -> dict[str, int]:
        return self._upsert(DimDistributor, "distributor_name",
                            "distributor_id", names)
    ...
    def seed_rating_sources(self) -> dict[str, int]:
        existing = {r.source_name: r.source_id for r in
                    self.session.exec(select(DimRatingSource)).all()}
        for source_id, name, scale_max, scale_unit in RATING_SOURCES:
            if name not in existing:
                self.session.add(
                    DimRatingSource(source_id=source_id, source_name=name,
                                    scale_max=scale_max, scale_unit=scale_unit)
                )
    ...
    def ensure_unknown_members(self) -> None:
        ...
        if self.session.get(DimDistributor, UNKNOWN_ID) is None:
            self.session.add(DimDistributor(distributor_id=UNKNOWN_ID,
                                            distributor_name="Unknown"))
            self.session.flush()

    def distributor_map(self) -> dict[str, int]:
        return {d.distributor_name: d.distributor_id for d in
                self.session.exec(select(DimDistributor)).all()}
