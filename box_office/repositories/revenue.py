...
class RevenueRepository:
    ...
    def load_csv(self, path: str | Path) -> int:
        ...
        self.session.execute(delete(BronzeRevenueCsv).where(BronzeRevenueCsv.source_file == name))

        table = BronzeRevenueCsv.__tablename__
        conn = self.session.connection()
        step = max(1, total // 20)  # ~5% chunks
        for start in range(0, total, step):
            df.iloc[start : start + step].to_sql(table, conn, if_exists="append", index=False)
            done = min(start + step, total)
            logger.info("csv rows %d/%d (%d%%)", done, total, done * 100 // total)
    ...
    def distinct_titles(self) -> list[str]:
        return list(self.session.exec(select(BronzeRevenueCsv.title).distinct()).all())
    ...
    def distinct_distributors(self) -> list[str]:
        rows = self.session.exec(select(BronzeRevenueCsv.distributor).distinct()).all()
        return [d for d in rows if d]

    def distinct_event_dates(self) -> list[date]:
        return list(self.session.exec(select(BronzeRevenueCsv.event_date).distinct()).all())
